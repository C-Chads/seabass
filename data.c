...
void print_manpage(char* subject){
    ...
    m(for_c_programmers){
        ...
        ll("  this is not an issue for performance. In fact, pass-by-value is vastly more expensive")
        ll("  for a 16 byte struct than pass-by-pointer.")
        ll("  So seabass code should actually be _faster_ by forcing pass-by-pointer for structs.")
