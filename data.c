...
void print_manpage(char* subject){
    ...
    m(reflection){
    	...
    	l("A support library is needed. Luckily, it is embedded within this very binary.")
    	l("You can ask for it by doing...")
    	nl
    ...
    m(parsehook){
        ...
        nl
	    ll("cbas -m builder_library_code")    
        nl
        l("note that it also requires the reflection library, which can be retrieved with:")
        nl
	    ll("cbas -m reflection_library_code")
        nl
    ...
    m(for_c_programmers){
        ...
        ll("  That this would have negative performance implications, however my own research")
        ll("  says that it is not a major issue. See System V AMD64 ABI section 3.2.3.")
        ll("  For static inline functions, your code should be optimized exactly as if you")
        ll("  had written the function with pass-by-value.")
        ll("  separately-compiled functions taking 9-16 byte arguments are")
        ll("  the only legitimate concern, and only becomes a problem if a function takes")
        ll("  a large enough number of arguments (So that using a 128 bit register would cause")
        ll("  an argument to be placed in a register instead of the stack). This should rarely")
        ll("  if ever cause a significant problem. It's bikeshedding.")
        ll("  If you need to call C code which takes a struct by argument, just write a wrapper")
        ll("  in C for the function which takes a pointer, and then implement its prototype")
        ll("  in Seabass.")
