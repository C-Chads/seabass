...
#include "parser.h"
/*
	this file contains all the codes that evaluates constant expressions down to single integers or floats.
*/


static int64_t cexpr_int_parse_int(){
	int64_t retval;
	require(peek()->data == TOK_INT_CONST, "constexpr error: Integer constant expression parsing expected integer constant...");
...
static double cexpr_double_parse_double(){
	...
	require(
		...
		, "constexpr error: Float constant expression parsing expected constant...");
...
static int64_t cexpr_int_parse_ident(){
	...
	int found;
	found = 0;
	require(peek()->data == TOK_IDENT, "constexpr error: cexpr_parse_ident: Required Identifier");
